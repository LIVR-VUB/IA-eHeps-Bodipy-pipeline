...
def _prepare_2d(img: np.ndarray, take_z: int | None) -> np.ndarray:
    ...
    if img.ndim == 3:
        # (Z,Y,X) or (C,Y,X)
        axis0 = 0
        if take_z is not None and img.shape[axis0] > 1:
            return img[min(max(int(take_z), 0), img.shape[axis0] - 1)]
        return img if img.shape[axis0] == 1 else img.max(axis=axis0)
    ...
    if img.ndim == 4:
        ...
        if take_z is not None and img.shape[0] > 1:
            return img[min(max(int(take_z), 0), img.shape[0] - 1)]
        return img if img.shape[0] == 1 else img.max(axis=0)
...
def _load_model(path: str) -> CAREamist:
    ...
    p = Path(path)
    # Most CAREamics versions accept path directly via source
    try:
        return CAREamist(str(p))
    except Exception:
        pass
    # Fallback: folder-based loading if available
...
def _predict_2d(model: CAREamist, x01: np.ndarray,
    ...
    # CAREamics may return np.ndarray directly or [np.ndarray]
    if isinstance(res, np.ndarray):
        return res
    if isinstance(res, (list, tuple)) and len(res) > 0:
        # sometimes nested like [array] or [ [array] ]
        r0 = res[0]
